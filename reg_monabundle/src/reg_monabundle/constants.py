...
inside ``mock_data_wizard.summarize``) so the namespaced-block
validator can enforce the "overrides may only raise the floor" rule
without importing mdw, and so the same constant is amalgamated into the
MONA bundle alongside the runtime modules that consult it.
