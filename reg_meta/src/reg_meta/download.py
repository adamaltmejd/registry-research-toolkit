"""Download pre-built reg_meta artifacts from GitHub Releases."""
...
PYPI_JSON_URL = "https://pypi.org/pypi/reg-meta/json"


@dataclass(frozen=True)
class ReleaseResolution:
    """Outcome of walking GitHub releases for reg_meta artifacts.

    *release_tag* is the literal tag of the latest reg_meta release.
...
def version_from_tag(tag: str) -> str:
    ...
    Handles both ``reg_meta/v0.5.0`` and legacy ``v0.5.0`` formats.
    """
    if "/" in tag:
        tag = tag.rsplit("/", 1)[1]
    return tag.lstrip("v")
...
def _is_reg_meta_release(release: dict) -> bool:
    """Match ``reg_meta/v*`` tags (and legacy bare semver-like ``v*`` tags)."""
...
def _pick_release(all_releases: list[dict]) -> ReleaseResolution:
    ...
    """
    releases = [r for r in all_releases if _is_reg_meta_release(r)]
    if not releases:
        raise RegMetaError(
            exit_code=EXIT_NETWORK,
            code="no_releases",
            error_class="network",
            message="No reg_meta releases found.",
...
def fetch_pypi_latest_version(*, timeout: float = 15) -> str:
    """Return the latest installable reg_meta version from PyPI.

    The package-version check for update prompts and `reg-meta update` must
    ...
    req = urllib.request.Request(
        PYPI_JSON_URL,
        headers={"Accept": "application/json", "User-Agent": "reg-meta"},
    )
    try:
        with urllib.request.urlopen(req, timeout=timeout) as resp:
...
def _download_file(url: str, dest: Path) -> None:
    headers = {"User-Agent": "reg-meta", **_github_auth_headers()}
