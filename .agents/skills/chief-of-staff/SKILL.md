...
rest, invoke the `file-issue` skill (`$file-issue`) with the entry — it re-runs the
dedupe search and files each genuinely-new one (an entry whose fresh search now matches
an existing issue is not filed). Collect the filed issue numbers for the report's
