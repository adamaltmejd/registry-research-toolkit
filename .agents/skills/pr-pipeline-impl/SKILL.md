...
- Python: `uv run ruff check`, `uv run ruff format --check`,
  `uvx --from ty==0.0.72 ty check`, and targeted `uv run python -m pytest <pkg>/`.
...
- **Real `build-db`** — for build-affecting work, the real `build-db` is a final gate on
  the truly final head — **last and once**, not part of the iterate loop. Run it per the
  **`build-db` skill** (timestamped log, post-build SQLite checks, long-session
  polling); the overlay-input rule for a PR that changes tracked
  `reg_meta_build/input_data/**` is canonical in **AGENTS.md "Real-data validation"**;
