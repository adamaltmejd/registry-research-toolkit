...
def write_auto_toml(
    ...
    An entry absent from ``derivation`` emits no comment — chiefly an
    existing-auto slug reloaded from a prior file (its VALUE round-trips via
    ``tomllib`` but the ``# source:`` comment does not). Pre-v1 every build
    regenerates from scratch under UNFROZEN, so ``derivation`` covers every slug
    and the file is fully marked. The §5.4-freeze incremental path (a committed
    auto.toml + only new slugs appended) would rewrite the pre-existing rows
    markerless; carry the prior markers forward via ``read_auto_derivations``
    when that path lands (#141).
    """
    lines = [
...
def populate_variable_slugs(
    ...
    for provider_slug in _live_providers(conn):
        ...
        # A4.4a: source_id → derivation class for slugs assigned THIS build (the
        # `# source:` comment in the rewritten auto.toml + the name-fallback
        # worklist basis). Only first-sight (Pass-3) slugs get a class — an
        # existing-auto slug is read back verbatim, its original derivation is
        # not recoverable from the value, so it stays absent (no comment).
