...
def populate_slugs(
    ...
    *,
    ...
    # `variable_state.slug` column via `populate_variable_slugs` (called
    # separately from `build_db`). `populate_slugs` itself only handles
    # register / variant / version / classification; it intentionally skips
    # variable rows here. The override gate that previously rejected
...
# ---------------------------------------------------------------------------
# Variable slugs (§5.3 stored `variable_state.slug`)
# ---------------------------------------------------------------------------
...
def _curated_variable_slugs(
    ...
    :func:`_auto_variable_slugs`. We separate the two so curated can win on a
    key clash regardless of file-load order.
    """
    out: dict[tuple[str, str], str] = {}
    for e in entries:
        if e.kind == "variable" and e.slug is not None and e.provider is not None:
            # Both files parse to `kind == "variable"`; tell them apart by
            # which file the entry came from is not recorded on SlugEntry, so
            # the caller passes only the curated file's entries here.
...
def _latest_delivery_column(
    conn: sqlite3.Connection, register_id: int, var_id: int
) -> str | None:
    """The era's latest `delivery_column_name` for a `(register_id, var_id)`.

    Latest = highest `valid_to`, lexically smallest `delivery_column_name` on
    ties — matching the coalescer's "latest alias" tie-break (db.py). Returns
    ``None`` when the variable has no non-NULL delivery column in any era.
    """
    row = conn.execute(
        "SELECT delivery_column_name FROM variable_state "
        "WHERE register_id = ? AND var_id = ? "
...
def populate_variable_slugs(
    ...
    *,
    ...
    """Populate `variable_state.slug` from curated + auto-derived slugs (§5.3).
    ...
    - **Auto** entries already recorded in ``<provider>.auto.toml`` — kept
      ...
      slug).
    - **First-sight auto-derive** for triples present in neither file: fold
    ...
    for provider_slug in _live_providers(conn):
        ...
        for register_id, regvar_id, var_id in triples:
            ...
            conn.execute(
                "UPDATE variable_state SET slug = ? "
                "WHERE register_id = ? AND var_id = ?",
                (slug, register_id, var_id),
            )
        ...
        if strict and unslugged:
            ...
            raise _err(
                ...
                f"{len(unslugged)} variable(s) under provider "
                f"{provider_slug!r} have no derivable slug: their latest "
                f"kolumnnamn folds to empty / reserved / period. {sample}.",
...
def _variable_source_slug(
    ...
    stores the canonical slug on `variable_state.slug` (denormalized per
    triple, so a single read suffices) — `populate_variable_slugs` runs before
    `materialize_same_as_edges`, so the column is populated here. This replaces
    the prior derive-from-aliases-and-require-unique workaround; the
    folding-ambiguity error it raised is now unreachable because a variable has
    exactly one stored slug. The "no slug for this pair" error is preserved for
    the genuine curation case (variable absent / underivable at build).
    """
    row = conn.execute(
        "SELECT slug FROM variable_state "
        "WHERE register_id = ? AND var_id = ? AND slug IS NOT NULL LIMIT 1",
        (register_id, var_id),
    ).fetchone()
    if row is None:
        raise _err(
            ...
            f"stored slug on `variable_state`, so its variable_slug can't be "
