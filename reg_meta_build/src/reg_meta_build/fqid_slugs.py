...
# ---------------------------------------------------------------------------
#
# A2.1.5-SEED (harvested from PR #133, commit 6759de60). This engine derives,
# persists with never-recompute immutability, and writes the stored FQID
# variable slug. It is DORMANT on this branch: not wired into `build_db` and
# not exercised by live tests, because re-pointing it onto the promoted
# register-unique `variable.slug` needs the `variable` table that A2.1.5 itself
# creates. The SQL below still targets the per-variant `variable_state.slug`
# from #133 — every such reference is a TODO(A2.1.5) → `variable.slug`
# (register-unique). Re-point checklist + build wiring: see `A2.1.5-SEED.md`.
...
def populate_variable_slugs(
    ...
    *,
    ...
    """Populate the stored variable slug from curated + auto-derived slugs (§5.3).
