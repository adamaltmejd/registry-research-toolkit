...
def write_auto_toml(
    ...
    ``derivation`` (A4.4a) maps ``source_id`` → derivation class (see
    ``_DERIVATION_KINDS``). When present, each ``slug = "…"`` line carries an
    inline ``# source: <class>`` comment recording HOW the slug was derived — a
    ...
    ``--providers=scb`` build stays byte-identical in its slug VALUES. An entry
    absent from ``derivation`` (e.g. an existing-auto slug read back from a prior
    file) emits no comment.
    """
    lines = [
...
# `_uniquify` is an orthogonal flag (`DerivationKind.disambiguated`), not a
# separate class.
