...
def _build_parser() -> argparse.ArgumentParser:
    ...
    suspects_p = sub.add_parser(
        ...
        description=(
            ...
            "triage siblings (variables sharing one (register, provider_key)) whose\n"
            "representative shapes disagree in a way that suggests one delivery was\n"
            "mis-imported — a numeric-vs-text type_flip, or a length_disagree on an\n"
            "unclassifiable type. #800 retired the `related` edge and its\n"
            "import_bug_suspect kind; this re-derives the signal READ-ONLY from\n"
            "variable_state for the deferred fold-or-sever curation. Reads a built DB;\n"
            ...
            "suspect reason, and whether the pair is already co-grouped (shares a\n"
            "concept_group — the resolved fold). The shape classifier is the SAME\n"
            "numeric/text/other classifier the build-time fold gate uses.\n\n"
