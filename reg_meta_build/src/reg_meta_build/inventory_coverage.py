...
A miss on ANY OTHER provider is never rendered in that grammar. `curation/scb_errata.toml`
...
* An omitted row names the `Registerversionnamn` the catalog carries VERBATIM
  ...
  `curation/scb_errata.toml` is authored in. A held edition no documented version covers
...
# ONE provider `curation/scb_errata.toml` accepts, imported rather than restated so the
...
class CoverageMiss:
    ...
    @property
    def scb(self) -> bool:
        """Is this miss on the one provider `curation/scb_errata.toml` accepts?"""
...
class CoverageReport:
    ...
    def scb_misses(self) -> tuple[CoverageMiss, ...]:
        """Every miss on the provider `curation/scb_errata.toml` accepts."""
...
def errata_stanzas(misses: Sequence[CoverageMiss]) -> str:
    """The loadable candidates in `misses` as an `curation/scb_errata.toml` fragment:
...
def errata_worklist(report: CoverageReport) -> str:
    """`report` as a candidate `curation/scb_errata.toml` worklist for a maintainer.
    ...
    lines = [
        ...
        "# reg_meta_build/curation/scb_errata.toml entries for the columns the steward holds",
...
# that provider is repaired in, since `curation/scb_errata.toml` refuses it. Built from the
...
def _curated_surface(provider: str) -> str:
    ...
    `curation/scb_errata.toml` cannot take.
