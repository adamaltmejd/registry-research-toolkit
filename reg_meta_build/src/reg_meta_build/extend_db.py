...
variable, or minting a shared SCB column SCB never documented — is
GLOBAL-build work and lives in the shipped mechanisms
(``delivery_enrichment.py`` for descriptions and aliases, ``scb_errata.toml``'s
``[[column]]`` for the columns), NOT here: a fact about a global register
belongs in the global build regardless of which steward surfaced it.
...
# window — mirrors the DDL `valid_to` default.
...
def extend_db(
    ...
    *,
    ...
    try:
        ...
        # overlay never inserts a `value_code` row (aliases/steward
