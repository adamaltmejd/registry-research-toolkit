...
def _load_var_signals(conn: sqlite3.Connection) -> dict[str, _VarSignals]:
    ...
    for row in rows:
        name = row["name"]
        norm_name = (
            name.strip().casefold() if isinstance(name, str) and name.strip() else None
        )
...
def infer_same_as_candidates(
    ...
    *,
    ...
    # suppression count is the scored-output difference between them. When the
    # cap is disabled no group is ever a hub, so the two sets are identical and
    # the delta is always empty — skip the uncapped set entirely.
    cap_active = max_signal_fanout is not None
    ...
    # output). Scoring is per-pair (deterministic on a pair's signals), so the
    # uncapped-minus-capped output difference equals scoring ONLY the pair delta
    # — no need to re-score the full (up to ~48k) uncapped set and set-diff. A
    # pair suppressed by one hub signal but generated by a non-hub signal is in
    # `capped_pairs` already (so not in `delta`), so it is correctly NOT counted.
    delta = uncapped_pairs - capped_pairs
    if not delta:
