...
from reg_meta_build.resolution import (
    Claim,
    SweepHooks,
    assemble_runs,
...
class _StateGroup:
    ...
    # #271 PR-A: one `Claim` (resolution.py) per REFERENCE year this group was observed in
...
#              sibling, and record the sibling pairs for the concept-group fold.
...
class _TriageResult:
    ...
    # (variable_id_a, variable_id_b) split-sibling pairs — the IN-BUILD sibling
    # sets the concept-group edge fold reads directly (`edge_siblings`). These are
    # NOT persisted to any shipped table; they only feed the #303 fold. The
    # researcher-facing `variable_related_to` edges (and the per-pair relation_kind
    # taxonomy) were retired, so a split now records only the bare sibling pairing
    # the fold needs.
...
def _cluster_contested(contested_cols: list[str]) -> list[list[str]]:
    ...
    for members in comps.values():
        ...
        if len(members_sorted) > 1 and whole_folds:
...
def _triage_groups(
    ...
    for (register_id, var_id, preset_variable_id), gkeys in by_var.items():
        ...
        if _decide_fold_or_split(folded) == "fold":
            ...
            clusters = _cluster_contested(contested_cols)
...
def _split_off_non_contested(
    ...
    Give each such column its own variable, recording the sibling pair so the
    concept-group fold can still re-collapse them into one browse row. Perfect
    rename *continuity* (re-joining a renamed column to its sibling) needs A2.3
    ...
    # Sibling pairs → the concept-group edge fold (`edge_siblings`); a
    # non-contested column is a temporal/rename variant of the same definition.
    for i, a in enumerate(vids):
        for b in vids[i + 1 :]:
            res.sibling_edges.append((a, b))
...
# When a `(variable, variant)` has OVERLAPPING distinct-value-set groups, its
# states are rebuilt from the OBSERVED edition claims (`_StateGroup.claims`)
# at INTERVAL grain by the provider-blind engine in `resolution.py` (drift
...
def _resolve_year_winners(
    ...
    the SCB adapter onto the provider-blind engine (`resolution.py`): the
    hooks close over this adapter's state groups, value-set accessors, and
    curation context; the engine owns the conflation/segmentation mechanics.
    ...
    middle). ``genuine`` lists `(column, seg_lo, seg_hi, gkeys)` segments
    still holding >1 distinct value set after the cascade and curation — the
    ...
    hooks = SweepHooks(
        ...
        resolve_pool=lambda pool: _resolve_column_year(
            pool, groups, year, codes_fn, codelivery, labels, code_labels_fn
        ),
    )
    cadence = _CADENCE_BY_REGISTER.get(cands[0][0], "year")
    return resolve_year_intervals(cands, hooks, cadence)
...
def _coalesce_variable_states(
    ...
    # Delivery correction provenance is an annotation on the availability the
    # coalescer would already produce, never an input to shape selection or the
    ...
    def _provenance_target(row: sqlite3.Row) -> tuple[int, int, str] | None:
        ...
        return None if vid is None else (vid, row["register_variant_id"], gkey[8])
    ...
    for row in rows:
        ...
        if target not in corrected_targets:
            continue
        claims = register_edition_claims(row["register_id"], row["registerversionnamn"])
        bounds = (
            [(lo, hi) for _year, lo, hi in claims]
            if claims
    ...
    def _partition_provenance(
        gkey: tuple, vid: int, vf: str, vt: str
    ) -> list[tuple[str, str, str | None]]:
        """Split one already-resolved state only where its attribution changes."""
        ...
        for index, lo in enumerate(starts):
            ...
            if segments and segments[-1][2] == provenance:
                ...
                segments.append((lo, hi, provenance))
        return segments
    ...
    def _append_state(
        ...
        for part_from, part_to, provenance in _partition_provenance(gkey, vid, vf, vt):
            if part_to == _VALID_TO_OPEN_SENTINEL or part_from == _VALID_FROM_UNKNOWN:
                sentinel_count += 1
    ...
    for (vid, rv), gkeys in by_vv.items():
        ...
        for gk, ints in owned.items():
            ...
            runs = assemble_runs(ints, rivals)
    ...
    return {
        ...
        # values): fold-slug hints → populate_variable_slugs; split-sibling pairs →
        # the concept-group edge fold (`edge_siblings`).
        "_fold_slug_hints": triage.fold_slug_hints,
        "_sibling_edges": triage.sibling_edges,
...
class SCBAdapter:
    ...
    attributes after `emit()` drains:
      ...
      - `sibling_edges` — split-sibling (variable_id, variable_id) pairs the
        ...
        routing, not an IR object and not persisted to any shipped table.
    """
