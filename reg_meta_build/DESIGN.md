...
into seven families:
  ...
  | **document provenance** | `doc_sources.toml`, `related_documents.toml` (loaded by `doc_db.py`)                                                                                                                                                                                                         | Maintainer-curated provenance for the separate doc DB. `doc_sources.toml` resolves converted markdown docs back to public SCB source PDFs without republishing them. `related_documents.toml` maps register-version related-document PDFs that may be rehosted verbatim, with per-file license, fetch date, source surface, and filename; binaries stay gitignored under `input_data/SCB/docs/<register>/` and are stored in the released doc DB. |
...
- **(a) Curation-only worklist** — the generator emits an *ephemeral* review worklist
  ...
  which ships empty and grows one reviewed entry at a time. For discovery-surface facts
  (where a wrong entry is a reversible mis-link, not data corruption) that are produced
  by an imprecise rule — the curation bar is reasonable-confidence cross-register
  identity (recall-liberal, bounded by the `_SAME_AS_MAX_COMPONENT` size cap), not
...
5. Loads `reg_meta_build/related_documents.toml` (#740). Each mapped binary present
   under `input_data/SCB/docs/<register>/` is stored verbatim in `related_document` with
   title, filename, source surface URL, license, fetched date, sha256, and byte size.
   Missing mapped binaries and unmapped register-subdir PDFs are build warnings; entries
   for registers absent from both the markdown docs and binary seed tree are skipped so
   curation can land before every register has local assets.
