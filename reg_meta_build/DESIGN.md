...
- **(a) Curation-only worklist** — the generator emits an *ephemeral* review worklist
  ...
  this surface ships empty. The accepted entry still resolves endpoints at build time
  when both endpoint providers are included; slug-anchoring preserves the authored FQID,
  not a dangling edge.
