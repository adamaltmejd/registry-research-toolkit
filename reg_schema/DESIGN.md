...
- The `project_data.json` v2 shape (Model A): Pydantic v2 models for `ProjectData`,
  ...
  `PeriodRange`) or — since #307 — a **list of segments** (an interrupted series, e.g.
  ...
  pseudo-sources). The structural list rules: non-empty, members are segments (a nested
  list is not), each member non-inverted, and the members **sorted ascending and
  non-overlapping** (adjacency allowed — the list expresses interruption; rejecting
  contiguity would need calendar adjacency math for no safety gain). Sorted-and-disjoint
  keeps the comma-joined wire form (`2005..2010,2015..2020`) canonical and per-segment
  resolution deterministic. Composite `entity_key` / `time_key` arrays are part of the
  schema from day one (the validator enforces their ordering/homogeneity rules).
  Remaining: composite-key runtime support in the extract path — see `REFACTOR_SPEC.md`.
...
Every v1 researcher project must carry an explicit requested period: the whole-history
`"_default"` sentinel was removed from `Source.period` once its only author — the
provisional steward pseudo-project — migrated to the delivery inventory, rather than
retaining a structurally valid but non-orderable project state. The SPA may expose a
common study window as an authoring default, but each `Source` still persists its
concrete period. Adding a source defaults that period to the full available
intersection, including disjoint segments. If the intersection is empty, the add is
blocked rather than inventing a period. A later common-window edit does not mutate
existing source periods; if it leaves an existing source disjoint, the source keeps its
explicit period and the project becomes blocking. Divergence remains visible rather than
being hidden as inheritance, and an explicit apply-to-all action rewrites only sources
with an overlap. An empty project is a valid editable draft but cannot be materialized
as an order. See `REFACTOR_SPEC.md` §12.
...
UI affordances, new codes are additive (§6.8.0). Current codes:
  ...
  | `invalid_period`                       | A `Source.period` is not an int year, a period-token string (`YYYY`, `YYYY-MM`, `YYYY-MM-DD`, `HTYYYY`, `VTYYYY`, `LA<YYYY>`, `YYYY-Q[1-4]`, `YYYY-H[12]`), a `{"from","to"}` range object with valid endpoints, or a #307 segment LIST (interrupted series). The list rules raise the same code (member-pathed `/period/<i>`): empty list, a non-segment member (nested list / junk), an inverted member range, or members not sorted-ascending / overlapping. A `YYYY-MM-DD` token that passes the syntactic 01-31 day envelope but names a calendar-impossible day (`2019-02-29` in a non-leap year, `2018-02-30`) also raises this code. |
...
here so the stable-code registry is complete and the SPA can map them:
  ...
  | ------------------------------------- | ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
  | `period_outside_state_validity`       | error                                 | No `variable_state` covers the binding at the source's variant ANYWHERE in the requested period. PARTIAL availability is not this code — it clips (`range_period_partially_covered`); only an entirely unavailable binding blocks.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
  | `binding_state_drifts_within_period`  | info                                  | The requested period crosses a state transition (incl. a delivery-column rename); the resolver returns per-state subsets. Not a coverage statement — a shortfall against the request is the clip below.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
  | `range_period_partially_covered`      | info                                  | The binding is available for only PART of the requested period (e.g. SSYK first delivered 2014 under a `from:2010,to:2020` binding). Under §12 intersection semantics it is requested wherever it IS available, so it is ordered for that part and this reports the clip rather than dropping it silently. ONE issue per binding for the whole request, naming the ordered period: a leading/trailing shortfall, an internal gap, an uncovered segment of a list period, and a pinned `representation` narrower than the request are all this same clip. It mirrors the `ClipReport` the order materializer emits from the same shared pass, so validation and ordering agree on what is available. Informational BY ITSELF only: a clipped binding can still fail its representation/value-set checks here, and the order's physical coverage gate there. Zero availability is `period_outside_state_validity`, not this. |
  | `binding_value_set_version_ambiguous` | error                                 | A binding's `(variant, period)` resolves to several **CO-EXISTING delivery columns** (distinct columns valid at the SAME REQUESTED instant — windows overlapping inside the request, so a sibling overlapping only in a HOLE of a list period is not ambiguity) — parallel REPRESENTATIONS of the one concept (SSYK 3/4/5-digit, age brackets) — and the binding sets no `representation`. The author must pick one (the SPA offers a chooser); this is where the retired `@version` pin's job now lives, keyed on the delivery column. Distinct columns in NON-overlapping windows (a sequential rename) are drift, not ambiguity. (Also re-used as a backstop for the rarer case of distinct value sets co-delivered on ONE column — a reg_meta build co-delivery the `validate` invariant should make unreachable.)                                                                                                     |
...
Two further codes reach that layer's output under the **materializer's** own spelling,
because `/api/project/validate` runs REFACTOR_SPEC.md §12's shared availability pass
rather than a second one: `representation_unresolved` (a state with no delivery column)
and `period_not_orderable` (a source period that cannot be expanded). They are §12
finding codes, listed here so the SPA's map is complete; the four codes the same pass
raises that DO have a §6.8.3 spelling are translated to it (e.g. `binding_unavailable` →
...
plus `HT/VT`, school-year `LA`, quarter, half-year forms). Both copies also
calendar-validate the author-supplied day of a `YYYY-MM-DD` token (`_is_period_endpoint`
here mirrors `is_period` on the reg_meta side): the regex bounds the day 01-31
syntactically, but an impossible day (`2019-02-29` in a non-leap year, `2018-02-30`) is
rejected by an extra `date.fromisoformat` check. The grammar is kept **bound-for-bound
identical** so a spec that passes this structural gate doesn't later fail reg_meta's
period resolution — a looser copy would silently split the period contract across the
two packages. #307 widened the mirror by one function: `structural._endpoint_bounds`
duplicates `reg_meta.fqid.period_token_to_bounds` (token → inclusive ISO interval,
including the deliberate synthesized Feb-29 upper bound) because the period-list
sorted/non-overlap rule needs real interval comparisons across mixed grammars (`HT2018`
⊂ `2018`). The cross-grammar parity test
(`reg_webapp/backend/tests/test_period_grammar_parity.py`) is the CI gate that enforces
this invariant — token verdicts AND bounds expansions: any future change to one side
that breaks parity with the other will fail CI.
