...
name: file-issue
description: >-
  File one GitHub issue with tracker hygiene enforced — re-run the dedupe search, verify
  the required area + type labels and an origin Relationships block, then create it. Use
  when asked to file a drafted issue, or when the chief-of-staff files a follow-up
  recorded in a pipeline merge-gate `followups.md`. Usage: /file-issue <inline draft, or
  a path to a followups.md entry/file>
argument-hint: "<inline draft, or a path to a followups.md entry/file>"
---

# File one issue

File the drafted issue below to the AGENTS.md **Issue tracker** conventions, then report
the created URL:

> $ARGUMENTS

This skill files ONE issue at a time. The chief-of-staff invokes it (via the `Skill`
tool) to file a follow-up recorded in a pipeline's merge-gate `followups.md`; a human
may invoke it directly on an inline draft.

## Read the draft

The argument is either the drafted issue inline, or a path to a `followups.md` file /
one `## <title>` entry inside it. If it's a path, read that file and take the entry to
...
Before creating anything, confirm all of these. If any can't be satisfied from the
draft, **refuse** and report exactly what's missing — do not invent labels or
relationships.
...
3. **A `Relationships` block wiring the issue to its origin.** A draft filed from a
   ...
   `Blocked by`. No origin tie ⇒ refuse.

## File it

Write the ready-to-file body to a temp file (from a `followups.md` record, that's the
