...
case "$file_path" in
*/regmeta/docs/lisa/*.md | regmeta/docs/lisa/*.md)
	cat >&2 <<EOF
Refused: $file_path is a generated build artifact under regmeta/docs/lisa/.
Edit scripts/parse_lisa_docs.py and rebuild the docs instead.
(See CLAUDE.md → "regmeta/docs/lisa/*.md are build artifacts".)
EOF
	exit 2
	;;
esac
