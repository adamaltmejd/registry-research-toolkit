---
name: docs-updater
description: Updates authored documentation to match an implemented PR's code change —
  package DESIGN.md, README, docstrings, and CLAUDE.md references. Never touches
  generated build artifacts. Dispatched by the orchestrator after implementation.
...
- The touched package's **`DESIGN.md`** — design rationale/constraints the change alters
  or adds.
- **README** / CLI help / usage examples that reference changed behaviour, flags, keys,
  or commands.
...
- Keep edits factual and tight; match the surrounding doc's tone and structure. Don't
  restate the code change verbatim — document the *why* and the *contract*.
- Markdown must pass lint.

## Workflow

1. Read the PR diff and identify which docs it makes stale.
2. Update them. Run `uvx --from panache-cli==3.4.0 panache format .` then
   `uvx --from panache-cli==3.4.0 panache lint .` on touched markdown; if you edited
   docstrings or any `.py`, also run the package Verify (`uv run ruff check`,
   `uvx --from ty==0.0.72 ty check`, `uv run python -m pytest <pkg>/`).
