...
- **`reg_monabundle` vs `reg_mockdata`** — the MONA bundle (built by the
  ...
  surface (`runtime.*`) amalgamated into the bundle. Heavy deps
  (duckdb/pyodbc/numpy) live behind a `runtime` extras group; the webapp
  container never installs them.
...
- **Stateless server.** No process-local caches that change behavior
  across requests; the catalog DB is read-only and `mmap`'d.
- **OpenAPI is the canonical contract.** `reg_webapp/backend/openapi.json`
  is committed and regenerated by `scripts/gen_openapi.py`. CI guards
  drift with a pytest **snapshot test** (`tests/test_openapi_snapshot.py`)
...
[`reg_webapp/DESIGN.md`](reg_webapp/DESIGN.md) §9.4/§9.5.
...
3. **Bundle determinism** — same spec → byte-identical `.py`. Invariant
   holds (ast.unparse amalgamation) but the test is **not yet written**
   (issue-tracked).
...
6. **MONA-shape integration** — a `sqlserver-linux` Docker container
   running the bundle's extract SQL end-to-end. **Remaining** (only
   string-level dialect tests exist today; the `@pytest.mark.integration`
   marker is used by `reg_meta`'s install test).
