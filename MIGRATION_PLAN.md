...
**Gate to A2.6**: A2.4 *and* A2.5 must merge. A2.6 flips the FQID grammar (drop the variant from the binding) + resolver flip + the resolver-side `same_as` traversal rewrite (the `same_as` **table** demotion already landed in A2.1.5), which requires both the new lineage tables (A2.4) and the new catalog API (A2.5) in place. No reverse dependency — A2.4 reads the already-demoted table, so the gate is acyclic.
...
### [ ] A2.6 — Drop period & variant from FQID grammar (resolver flip + same_as resolver rewrite)
...
the stored variable slug, **and the `same_as` table grain-demotion**);
A2.6 flips the FQID grammar to match — the 3-segment binding FQID names
the variable directly — plus the resolver flip and the resolver-side
`same_as` traversal rewrite. No table rename, re-parent, or column drop
here (all done in A2.1.5).
...
- **Rewrite the resolver-side `same_as` traversal (respec).** The `variable_same_as` **table** is already variable-grain (A2.1.5 dropped `a_variant`/`b_variant` + `a_period`/`b_period` and deleted the `auto:var_id_match` `(N choose 2)` auto-derive; the name was kept). A2.6 rewrites the **query side**: `_resolve_binding_via_same_as` in `reg_meta/catalog.py` — the start node is the binding's variable; the BFS traverses variable-slug triples; no variant/period carried through. Remove the empty-string-sentinel variant/period fallback (current query: `AND (a_variant = '' OR a_variant = ?)` / `AND (a_period = '' OR a_period = ?)`). `_var_same_as_source_keys` shrinks to a 3-tuple `(provider, register, slug)`. (The row-count collapse from tens of thousands to the low-hundreds curated cross-register set happened at A2.1.5 with the auto-derive deletion.) `classification_same_as` already lost any `*_period` columns in A2.1.5.
