...
- **Known interim limitations (flagged for review; not correctness bugs — build + resolver + index all hold):**
  ...
  - **Version-specific slug for unsplit column-drifting variables** ([#143](https://github.com/adamaltmejd/registry-research-toolkit/issues/143)) — sibling facet of #141 but *unsplit*: a 1:1 variable whose single `delivery_column_name` drifts across editions auto-slugs from the latest column (e.g. `slk/sun2020inr1` for a var named "Utbildningsinriktning" that was SUN96→SUN2000→SUN2020). Fix = slug from name/earliest column when the column isn't constant across states + regenerate while UNFROZEN. Surfaced by the A2.3 `replaced_by` edge in #140.
