...
- **A5.2b-ii (this PR, [#174](https://github.com/adamaltmejd/registry-research-toolkit/pull/174))** — `/api/project/validate` (raw-dict body → §6.8.0 structural ⧺ block ⧺ semantic, 200 + `ok:false` on a failing spec, 4xx only on a malformed REQUEST) + `/api/project/order` (default CSV, structural-gated, formula-injection-neutralized) + `/api/bundle` (the 3-call reuse chain, embeds the RAW dict → preserves steward-namespaced blocks). All three read the raw body async → `run_in_threadpool` (blocking work off the event loop; per-request reg_meta conn on the worker thread). Write-side §9.4/§16 hardening: streaming body cap (1 MB, byte-counted) + stdlib per-IP token-bucket. **Review was intensive** — independent panel (2 P1: deep-nested→500, CSV formula-injection; 2 P2) + Codex (3 P2 + 1 false-positive) + Copilot (4) — all resolved + regression-tested. **OPEN (maintainer): the validator-interface decision** — relocate the webapp-composition codes (`invalid_field`, `invalid_block`) to issue-based reg_schema (`validate_structural` `extra="forbid"` + a canonical structural code) / reg_monabundle (issue-based `validate_block` + cross-block) validators, which ALSO reconciles the `/validate`↔`/bundle` gate divergence (bundle runs cross-block + capability gates `/validate` doesn't). Churn-free before A5.3 (no consumer maps the codes yet).
- **A5.2c** — `/api/kit` + `project_data.codes.json`, DEFERRED to §15 step 8 (open).

**A5.2 read + write surface COMPLETE** (A5.2a + A5.2b-i + A5.2b-ii merged); only A5.2c (kit, deferred) remains, so the box stays unchecked but **A5.3 (SPA, codegens against A5.2's OpenAPI) is unblocked**.

**Validator-interface cleanup (this PR)** — resolves the b-ii "OPEN" decision above, before A5.3 so the SPA maps canonical, validator-owned §6.8.0 codes. reg_schema `validate_structural` now emits `unexpected_field` (error) on the CLOSED objects (Source/Binding/Panel/member; top level stays open for namespaced blocks); reg_monabundle exposes BUILD-side issue forms (`block_issue` → `invalid_block`; `column_options_issues` → `column_options_orphan_fqid` / `suppress_k_on_non_categorical`) while `validate_block` stays raise-based + amalgamation-safe (untouched). reg_webapp `/api/project/validate` drops the invented `invalid_block` and the extra-key `invalid_field` adaptation (now consumes the canonical issues; the extra-key case is structural's `unexpected_field`, while `invalid_field` survives only as a rare defensive model-construction catch) and runs the cross-block check — closing the `/validate`↔`/bundle` gate divergence for that class (the step-4 capability gate stays an intentional lenient residual: `/validate` defaults `display_name` from reg_meta). New cross-runtime `test_corpus/unexpected_field_on_binding` case is the contract A5.3's TS structural validator must match.
...
### [x] A5.3 — SPA TypeScript regen

**Split into 3 PRs (maintainer): A5.3a (browse foundation) → A5.3b (states picker + lineage) → A5.3c (authoring + validation). Sequential A5.3→A5.4; path-based routing; Vitest.** **A5.3a MERGED (PR [#178](https://github.com/adamaltmejd/registry-research-toolkit/pull/178), squash 3915ff28) 2026-06-02** — the read-only browse SPA (was a context-only stub): typed fetch client + hand-rolled path router (`/catalog/...`, SPA-route-restricted interception, never-throws parse) + catalog tree over the kind-union + variant browser (register sub-resource) + the shared reactive `asyncResource` (synchronous `fn()` so `$effect` tracks inputs) + Vitest wired into CI (29 tests). **A5.3b MERGED (PR [#182](https://github.com/adamaltmejd/registry-research-toolkit/pull/182), squash 09befde1) 2026-06-02** — period/variant resolution + the multi-vintage states picker + the binding sub-endpoint panels on the binding leaf. Reactive router `search` signal (a `?period` change refetches without a remount); full node passed from CatalogNodeView so a cold deep-link with `?period` renders; clearable active-modifier chips (reversible narrowing); free-text period field. **TWO maintainer-approved §16 backend changes: `?value_set_version` accepts a FREE-TEXT label (matched by a Python filter in `resolve_at`, not SQL — the old slug gate 422'd every real label) + a `_none` sentinel (handler maps → `''`) so the empty/default-label state is selectable.** No OpenAPI/SCHEMA_VERSION change. **A5.3c split into c-i + c-ii (maintainer); focused authoring scope (sources + bindings); accept range schema_version 2.x + `reg_meta/v1.x`.** **A5.3c-i MERGED (PR [#185](https://github.com/adamaltmejd/registry-research-toolkit/pull/185), squash 1daeb632) 2026-06-02** — the project-authoring data layer + validation presentation + a minimal editor (this note rides in on the c-ii branch, per the tracker convention): `project_data.ts` (open-object `ProjectData` model + immutable source/binding mutators + `regMetaReleaseTag`), `project_store.svelte.ts` (**THE A5.4 SEAM** — `draft` + §9.7 dirty baseline, `openError` channel, the `validate`/`order`/`bundle` write calls, a debounced autosave `$effect` + load-at-init over a `ProjectPersistence` interface with an `InMemoryPersistence` stub [A5.4 swaps IndexedDB via `setPersistence`], and `checkVersionGate` accept-path-live with the marked v0.x-reject insertion point), `validation.ts` (RFC-6901 `parseJsonPointer` + corpus-pinned `KNOWN_CODES` presentation registry — backend stays canonical §9.6), `ProjectEditor`/`ValidationPanel` (home/new/open, validate + downloads gated on a clean validate, defensive against malformed opened specs), `api.ts` `apiPostJson`/`apiPostForBlob` (a validation FAILURE is 200 `ok:false`, not a 4xx) + shared `triggerDownload`/`errMessage`. **Maintainer-approved fork:** the new-project seed formats `reg_meta_version` into the canonical `reg_meta/v…` release tag (via `regMetaReleaseTag` from the bare `/api/context` package version) + sources `steward` from `context.steward.id`. Frontend-only (gen:types no-diff). **Review: Codex + Copilot + an independent panel** — a malformed-open render crash, a stale-`/validate` race (snapshot guard), + test-quality gaps (a vacuous test, missing download/validate coverage, an unsound corpus cast), all fixed + re-verified (144 vitest, live visual gate against the real reg_meta DB). **A5.3c-ii MERGED (PR [#188](https://github.com/adamaltmejd/registry-research-toolkit/pull/188), squash f349733) 2026-06-02** — the rich editable source/binding UI (this note rides in on the A5.4 branch, per the tracker convention): `SourceEditor`/`BindingEditor` (editable `register_variant` text field + variant picker; a type-conditional **Advanced disclosure** for the rare subtype/format overrides), `CatalogPicker` (inline-expand pick-mode reusing the catalog data layer; **derive-on-pick** resolves a picked variable at the source's `(period, variant)` and prefills `type` + `display_name`), year-range-first `PeriodEditor`, `FieldIssues` inline RFC-6901 highlighting via new pure `validation.ts` helpers (`jsonPointer` + `issuesUnderPointer`). Pure domain logic in tested `.ts` homes — `deriveType` (incl. the Swedish SOS `Datatyp` tokens), `periodToWire`, `registerPrefixOf`/`variantSeg`, `COLUMN_TYPES`. **Maintainer fork:** derive-core + Advanced disclosure (the catalog gives type/display-name/value-set-version). **`@version` pin DEFERRED** (maintainer: defer): the free-text `value_set_version_label` isn't a slug and the API doesn't expose the classification slug, so a co-delivered binding emits the bare FQID and the backend's `binding_value_set_version_ambiguous` flags it — proper pinning needs a backend slug-exposure follow-up. Frontend-only (gen:types no-diff). **Review: an independent panel + Codex** — PeriodEditor stale-state on indexed-source reuse (value-keyed re-seed), a cross-source radio-group collision (unique group name), the invalid version-pin (removed) — all fixed + re-verified live (163 vitest; real-DB visual gate: new → register_variant + Pick variant → year-range → add binding → Pick variable resolves + prefills `type=categorical` + `display_name=Kon`). **A5.3 COMPLETE** (a + b + c). The last stage is **A5.4** (the migration completion gate).
