...
**Real-data validation (full SCB `build-db`, schema 4.4.0).** Builds clean end-to-end: 118,547 `variable_state` rows; triage **3,173 folds / 6,980 splits / 281 collapsed**; uniqueness index holds; 58,185 valid variable slugs (0 invalid); 200,980 `variable_related_to` edges. Spot-checked the largest split (`hut`/`var_id 35989` "Imputerat", 169 siblings = `BANTALRUM_imp`/`BOAREA_imp`/`U0111_imp`…) — the canonical §5.7 generic-container split, correct. Two real-data bugs the fixtures missed were fixed (residual collapse on mixed `value_set_id`; invalid all-digit fold-slug stem).
