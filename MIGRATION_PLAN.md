...
**Validator-interface cleanup MERGED (this PR)** — the b-ii "OPEN" decision above, done before A5.3 so the SPA maps canonical, validator-owned §6.8.0 codes. reg_schema `validate_structural` now emits `unexpected_field` (error) on the CLOSED objects (Source/Binding/Panel/member; top level stays open for namespaced blocks); reg_monabundle exposes BUILD-side issue forms (`block_issue` → `invalid_block`; `column_options_issues` → `column_options_orphan_fqid` / `suppress_k_on_non_categorical`) while `validate_block` stays raise-based + amalgamation-safe (untouched). reg_webapp `/api/project/validate` drops the invented `invalid_field`/`invalid_block` (now consumes the canonical issues) and runs the cross-block check — closing the `/validate`↔`/bundle` gate divergence for that class (the step-4 capability gate stays an intentional lenient residual: `/validate` defaults `display_name` from reg_meta). New cross-runtime `test_corpus/unexpected_field_on_binding` case is the contract A5.3's TS structural validator must match.
