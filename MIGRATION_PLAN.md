...
- `SCHEMA_VERSION` rides the 4.x line (promotion + re-parent + column move; no consumer-visible break until A2.7).
...
- **`same_as` table-shape dependency (honest note).** The `same_as` table is **not** restructured by A2.1.5 — it is still the variant-grained `variable_same_as` (with `a_variant`/`b_variant` and the `(N choose 2)` var_id auto-derived rows) until **A2.6** demotes it to variable grain — dropping the variant columns and deleting the auto-derive. So `variable_set_via_same_as` has two clean options: (a) implement it reading the post-A2.6 `variable_same_as` and **order A2.4 after A2.6's same_as work** (then A2.4 has no lineage dependency on the legacy table at all), or (b) implement it now against `variable_same_as` projected to variable grain (group rows by `(provider, register)` endpoints, ignore the variant columns, dedup the auto-derived rows) and align with the demoted shape in A2.6. **(a) is cleaner and is the planned order** — A2.4's gate below already places it after A2.5, and A2.6's same_as demotion can land before A2.4's linker if convenient; pick (a) unless A2.4 is started early, in which case (b) is the documented fallback.
...
**Gate to A2.6**: A2.4 *and* A2.5 must merge. A2.6 flips the FQID grammar (drop the variant from the binding) + resolver flip + the `variable_same_as` grain demotion + var_id-auto-derive deletion, which requires both the new lineage tables (A2.4) and the new catalog API (A2.5) in place.
