...
import BindingLeafLifecycleHarness from "./BindingLeafLifecycleHarness.svelte";
...
describe("BindingLeafView representation picker (#678)", () => {
  ...
  it("does not stage source period replacements from a partial leaf view", async () => {
    ...
    // Following the link opens no review either: a source-period correction is an
    // explicit action (Y-15), never a consequence of where the researcher browsed.
...
//
// A researcher who already has a named source in the draft corrects ITS requested
// period, without deleting and rebuilding the source. Ordinary browsing (a year
// change, a row filter, a `?period` link) still stages nothing — that guard has its
// own regression here and in RepresentationPicker.browser.test.ts. The target is a
// NAMED source: the draft below carries two differently named sources on the SAME
// register variant, which only the source name can tell apart.
describe("BindingLeafView source-period correction (Y-15)", () => {
  /** `LISA` (2010..2015) with Kon — the leaf's own column — plus `scb/lisa/alder`,
   * which this leaf never shows; and `LISA_2`, the differently named sibling on the
   * same variant that must not move. Seeded through `updateField` because the
   * catalog's add path finds-or-creates by variant and so cannot author two. */
  ...
  const changeLisa = () => changeSourcePeriod("LISA");
  const applyPeriod = () => applySourcePeriod("LISA");
  const newPeriod = newPeriodInput;

  it("names both sources on this variant and reviews only the one chosen", async () => {
    seedTwoSources();
    await renderLeaf();
    ...
    await expect
      ...
      .toBeVisible();
    await expect.element(changeLisa()).toBeVisible();
    await expect.element(changeSourcePeriod("LISA_2")).toBeVisible();

    await changeLisa().click();

    // The review names its target and shows what a source-wide rewrite touches —
    // the old period, and `scb/lisa/alder`, a binding this leaf does not list.
    const review = page.getByRole("group", {
      name: "Change source period: LISA",
      exact: true,
    });
    await expect.element(review.getByText("2010..2015")).toBeVisible();
    await expect.element(page.getByText("scb/lisa/alder")).toBeVisible();
    await expect
      .element(page.getByText("Columns on this source (2)"))
      .toBeVisible();
    await expect.element(newPeriod()).toHaveValue("2010..2015");
    // Opening the review moves focus into the field the action named.
    await expect.element(newPeriod()).toHaveFocus();
    // Only one review at a time, and the sibling stays identified by name.
    await expect.element(changeSourcePeriod("LISA_2")).toBeDisabled();
  });

  it("applies a period-only diff to the named source, keeping its name and bindings", async () => {
    seedTwoSources();
    await renderLeaf();
    await changeLisa().click();

    // The current period is not a change — Apply stays disabled until it moves,
    // and the review says why rather than leaving a dead button unexplained.
    await expect.element(applyPeriod()).toBeDisabled();
    await expect
      .element(page.getByText(/already this source's period/))
      .toBeVisible();
    await newPeriod().fill("2012..2014");
    await expect.element(applyPeriod()).toBeEnabled();
    await applyPeriod().click();

    await expect.element(page.getByText(/1 period change/)).toBeVisible();
    expect(sourceShapes()).toEqual([
      ["LISA", { from: 2012, to: 2014 }, ["scb/lisa/kon", "scb/lisa/alder"]],
      ["LISA_2", 2009, ["scb/lisa/kon"]],
    ]);
    // The review closes on success; the list is back with the new period on it.
    await expect.element(newPeriod()).not.toBeInTheDocument();
    await expect.element(page.getByText("2012..2014")).toBeVisible();
  });

  it("Reset closes the review and changes nothing", async () => {
    seedTwoSources();
    await renderLeaf();
    await changeLisa().click();
    await newPeriod().fill("2012..2014");
    ...
    await expect.element(newPeriod()).not.toBeInTheDocument();
    // Closing hands focus back to the row that opened the review, rather than
    // dropping it at the top of the document with the unmounted button.
    await expect.element(changeLisa()).toBeEnabled();
    await expect.element(changeLisa()).toHaveFocus();
    expect(sourceShapes()).toEqual([
      ["LISA", { from: 2010, to: 2015 }, ["scb/lisa/kon", "scb/lisa/alder"]],
      ["LISA_2", 2009, ["scb/lisa/kon"]],
    ]);
  });

  it("refuses to stage an empty or structurally invalid period", async () => {
    seedTwoSources();
    await renderLeaf();
    await changeLisa().click();

    await newPeriod().fill("");
    await expect.element(newPeriod()).toHaveAttribute("aria-invalid", "true");
    await expect.element(applyPeriod()).toBeDisabled();
    await expect.element(page.getByText(/Enter a period/)).toBeVisible();

    // `2020,2019` looks like the grammar but breaks the sorted/non-overlapping
    // list rule the existing structural check enforces.
    await newPeriod().fill("2020,2019");
    await expect.element(newPeriod()).toHaveAttribute("aria-invalid", "true");
    await expect.element(applyPeriod()).toBeDisabled();
    await expect.element(page.getByText(/isn't a usable period/)).toBeVisible();
  ...
  it("does not clip the proposal to what this leaf delivers", async () => {
    // The leaf's own Kon row runs 2010–2015; a correction is a SOURCE-wide
    // instruction and must not be intersected with the current view's availability.
    seedTwoSources();
    await renderLeaf();
    await changeLisa().click();
    await newPeriod().fill("2005..2020");
    await applyPeriod().click();
  ...
  it("does not call a review stale because the page stopped covering its source", async () => {
    // Browsing narrows what a leaf shows; it does not move a source. The staleness
    // notice reads the DRAFT, so a review outlives the row that opened it — and
    // still commits, because nothing about the source changed.
    seedTwoSources();
    const { rerender } = await renderLeaf();
    await changeLisa().click();
    await newPeriod().fill("2012..2014");
    ...
    await expect.element(changeLisa()).not.toBeInTheDocument();
    await expect.element(staleReviewAlert()).not.toBeInTheDocument();
    await expect.element(applyPeriod()).toBeEnabled();
    await applyPeriod().click();
  ...
  it("commits the review on Enter in the period field", async () => {
    seedTwoSources();
    await renderLeaf();
    await changeLisa().click();
    await newPeriod().fill("2012..2014");
    // Enter submits the review's form. `requestSubmit()` runs the onsubmit handler
    // (a bare dispatch of a non-cancelable submit wouldn't) — the same idiom the
    // omnibox's Enter test uses.
    const form = (newPeriod().element() as HTMLInputElement).form;
    form?.requestSubmit();
  ...
  it("invalidates a review whose source changed under it", async () => {
    seedTwoSources();
    await renderLeaf();
    await changeLisa().click();
    await newPeriod().fill("2012..2014");

    // A binding arrives on the reviewed source: the reviewed period is no longer
    // the whole story, so the review must be re-read rather than applied.
    projectStore.applyStagedDiff({
      adds: [
        {
          registerVariant: "scb/lisa/individer",
          period: { from: 2010, to: 2015 },
          binding: { variable: "scb/lisa/inkomst", type: "numeric" },
        },
      ],
    });

    await expect.element(staleReviewAlert()).toBeVisible();
    await expect.element(applyPeriod()).toBeDisabled();
    expect(projectStore.draft?.sources[0]?.period).toEqual({
      from: 2010,
      to: 2015,
    ...
    // Reset is the way out, and a fresh review reads the source as it now is.
    await page.getByRole("button", { name: "Reset" }).click();
    await changeLisa().click();
    await expect
      .element(page.getByText("Columns on this source (3)"))
      .toBeVisible();
  });

  it("invalidates a review whose source is gone", async () => {
    seedTwoSources();
    await renderLeaf();
    await changeLisa().click();
  ...
  it("invalidates a review when the whole project is replaced", async () => {
    seedTwoSources();
    await renderLeaf();
    await changeLisa().click();
    await newPeriod().fill("2012..2014");

    // A New/Open: the replacement may carry an identical source, and this review
    // still is not its.
    projectStore.newProject({
      reg_meta_version: "reg_meta/v1.0.0",
      steward: "global",
  ...
  it("refuses an Apply whose target changed while the restore was still settling", async () => {
    // The lifecycle's restore is asynchronous, so an Apply waits for it. The
    // re-check runs AFTER that wait, immediately before the mutation — a source
    // that moved inside the window is refused, not overwritten.
    // A restore that settles only on `finishRestore` — the same shape the store's
    // own restore-race tests use.
    const { promise: loaded, resolve: finishRestore } =
      Promise.withResolvers<null>();
    setPersistence({ save: () => Promise.resolve(), load: () => loaded });
    projectStore.updateField("name", "already-authored");
    seedTwoSources();

    await render(BindingLeafLifecycleHarness, {
      fqidPath: "scb/lisa/kon",
      node: node(pickerStates),
      regMetaVersion: SEED.regMetaVersion,
      steward: SEED.steward,
    ...
    // Still waiting on the restore — nothing is written yet.
    expect(projectStore.draft?.sources[0]?.period).toEqual({
      from: 2010,
      to: 2015,
    });
    ...
    await expect.element(staleReviewAlert()).toBeVisible();
    // The competing write stands; the stale review wrote nothing over it.
    expect(projectStore.draft?.sources[0]?.period).toBe(2011);
    await expect
      .element(page.getByText(/1 period change/))
      .not.toBeInTheDocument();
  });

  it("keeps the correction and the picker's column staging mutually exclusive", async () => {
    seedTwoSources();
    await renderLeaf();

    // Staged columns first: a correction cannot be started on top of them, because
    // an add's period union reaches the same source.
    await page.getByRole("checkbox", { name: /Sni/ }).click();
    await expect.element(changeLisa()).toBeDisabled();
    await expect
      .element(page.getByText(/Apply or reset the staged columns/))
      .toBeVisible();

    // Clearing the staging hands the correction back.
    await page.getByRole("button", { name: "Reset" }).click();
    await expect.element(changeLisa()).toBeEnabled();

    // And with a review open, the picker's own staging and Apply are unavailable —
    // its staged picks are preserved, never discarded.
    await changeLisa().click();
    await expect
      .element(page.getByRole("checkbox", { name: /Sni/ }))
      .toBeDisabled();
    await expectApplyDisabled();
    // …and the picker says why its controls are inert, beside them.
    await expect
