...
`related_to` / `lineage` edges), plus the full variable `succession_chain` (#582,
below). `lineage_warnings` are **omitted** — `ResolvedVariable` doesn't carry them; they
arrive via the `/lineage_warnings` endpoint.
