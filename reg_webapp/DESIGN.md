...
- **`TechnicalDetails`** (#638 PR4) — the shared "Technical details" `<details>`
  ...
  source-edition/evidence association separately; the disclosure remains collapsed and
  ordinary states add no row. Concept groups still use the component for key / facets /
  source. One component keeps the summary + styling consistent across call sites;
  callers omit it entirely when there's nothing to demote. `LineageDetails` follows the
  same omit-when-empty rule: with no provenance, warnings, loading state, or error, it
  renders nothing.
