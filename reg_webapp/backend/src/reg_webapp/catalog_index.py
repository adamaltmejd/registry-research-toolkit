...
from .semantic import _parse_binding_variable
...
class CatalogIndex:
    ...
    def admits(self, fqid: str) -> bool:
        """True iff ``fqid`` (a binding FQID) is admitted by ANY variant in the
        index. Used by ``fqid_outside_steward_catalog`` (A5.2b-ii)."""
    ...
    for s_idx, source in enumerate(project.sources):
        ...
        for b_idx, binding in enumerate(source.bindings):
            ...
            if _is_dropped(source_base, binding_base, dropped):
                continue
            variant_bindings.add(_parse_binding_variable(binding.variable)[0])
