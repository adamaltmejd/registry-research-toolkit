...
# ── Panel inheritance (kit/bundle-build only — see reg_schema/DESIGN.md → ─────
# Effective-key presence is not structural) ──────────────────────────────────
# This is the §6.8.3 `panel_inheritance_unresolvable` check. It is DELIBERATELY
# NOT part of `validate_semantic` (the researcher-/steward-validate path): the
# structural layer keeps the SPA's pre-kit authoring spec VALID while panel keys
# are still unresolved (a member may legitimately have no override and no panel
# default mid-authoring, expecting the variant `panel_template` to fill it). Only
# kit-build MATERIALIZES that inheritance, so only kit-build can flag an
# inheritance that has nothing to resolve from. The kit endpoint calls this in
# addition to `validate_semantic(caller="researcher")`.


def validate_panel_inheritance(
    project: ProjectData, catalog: Catalog
) -> ValidationResult:
    """Every panel member must end up with an effective ``entity_key`` AND
    ``time_key`` once inheritance is materialized — from the member override, the
    panel-level default, or the member's variant ``panel_template`` (reg_meta).
    A member that resolves NONE for either key →
    ``panel_inheritance_unresolvable`` (error): the kit can't be built because the
    panel join column is unknowable. Never opens a connection (the caller owns the
    ``Catalog``)."""
    by_name = {source.name: source for source in project.sources}
    issues: list[ValidationIssue] = []
    for p_idx, panel in enumerate(project.panels):
        for m_idx, member in enumerate(panel.members):
            _check_panel_member(panel, member, p_idx, m_idx, by_name, catalog, issues)
    return ValidationResult(issues=tuple(issues))
...
def _check_panel_member(
    ...
    mbase = f"/panels/{p_idx}/members/{m_idx}"
    source = by_name.get(member.source)
    # A member naming an unknown source is `panel_member_unknown_source` from the
    # structural layer (kit-build gates on that first), so this is a defensive
    # guard — there is no variant to inherit from, but the structural error
    # already explains the real problem, so don't pile on.
    if source is None:
        return
    template_entity, template_time = _variant_panel_template(
        source.register_variant, catalog
    )
    eff_entity = _first_present(member.entity_key, panel.entity_key, template_entity)
    eff_time = _first_present(member.time_key, panel.time_key, template_time)
    if eff_entity is None:
        issues.append(
            _panel_unresolvable_issue(
                panel, member, f"{mbase}/entity_key", "entity_key"
            )
        )
    if eff_time is None:
        issues.append(
            _panel_unresolvable_issue(panel, member, f"{mbase}/time_key", "time_key")
        )


def _first_present(*values: object) -> object:
    """The first non-``None`` value (the inheritance chain: member override →
    panel default → variant template). ``None`` when every level is unset."""
    for value in values:
...
def _variant_panel_template(
    register_variant: str, catalog: Catalog
) -> tuple[object, object]:
    """The ``(panel_entity_key, panel_time_key)`` a variant's ``panel_template``
    contributes to inheritance, or ``(None, None)`` when the variant carries no
    template (most variants) or doesn't resolve. The variant coordinate is
    ``<provider>/<register>/<variant>``; we list the register's variants and match
    the slug (the same browse axis ``_check_register_variant`` uses)."""
    parts = register_variant.split("/")
    if len(parts) != 3:
        return None, None
    provider, register, variant_slug = parts
    for variant in catalog.list_variants(provider, register):
        if variant.slug == variant_slug:
            return variant.panel_entity_key, variant.panel_time_key
    return None, None


def _panel_unresolvable_issue(
    panel: Panel, member: PanelMember, path: str, key_name: str
) -> ValidationIssue:
    return ValidationIssue(
        level="error",
        code="panel_inheritance_unresolvable",
        path=path,
        message=(
            f"panel {panel.panel_id!r} member {member.source!r} has no effective "
            f"{key_name}: no member override, no panel default, and its variant "
            "has no panel_template to inherit from"
        ),
    )
