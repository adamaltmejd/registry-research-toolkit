...
def test_cache_control_for_per_route():
    ...
    # asserts a deploy version/date — a stale copy lies); fold- or
    # steward-dependent /api/catalog/*, /api/search, and /api/stats reads get the
    # short 60s window; the rebuild-stable /api/docs/* reads keep the 24h policy.
    ...
    # Stats are steward-dependent for filtered deployments (#726), so they also
    # need prompt revalidation after a same-id steward catalog redeploy.
    assert cache_control_for("/api/stats") == CACHE_CONTROL_SHORT
