...
Build pipeline for the regmeta SQLite databases (main + docs) plus the
slug TOML maintenance subcommands. The query CLI lives in `regmeta`;
...
def _build_parser() -> argparse.ArgumentParser:
    parser = NoRepeatParser(
        prog="regmeta-build",
        description="Build pipeline for the regmeta SQLite databases.",
    ...
    build_p = sub.add_parser(
        ...
        description=(
            ...
            "should use `regmeta update` to fetch the pre-built DB instead.\n\n"
            ...
            "  regmeta-build build-db --input-dir regmeta_build/input_data/ --skip-slugs"
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    ...
    build_docs_p = sub.add_parser(
        ...
        description=(
            ...
            "End users receive the doc DB via `regmeta update`; this command\n"
            "is for maintainers rebuilding from a repo checkout before upload.\n\n"
            "Examples:\n"
            "  regmeta-build build-docs\n"
            "  regmeta-build build-docs --docs-dir /path/to/docs/"
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    build_docs_p.add_argument(
        ...
        help=(
            ...
            "(default: regmeta_build/docs/ if run from a repo checkout)."
        ),
    )
    ...
    seed_slugs_p = sub.add_parser(
        ...
        description=(
            ...
            "Examples:\n"
            "  regmeta-build seed-slugs\n"
            "  regmeta-build seed-slugs --out-dir /tmp/slugs/"
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    ...
    parse_sos_p = sub.add_parser(
        ...
        description=(
            ...
            "Examples:\n"
            "  regmeta-build parse-sos input_data/Socialstyrelsen/\n"
            "  regmeta-build parse-sos input_data/Socialstyrelsen/PAR.xlsx"
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
        ...
        if validation.failures:
            raise RegmetaError(
                ...
                remediation=(
                    ...
                    "`regmeta-build build-db --validate`."
                ),
            )
    ...
    if args.docs_dir:
        ...
        if docs_dir is None:
            raise RegmetaError(
                ...
                remediation=(
                    "Run from a regmeta_build checkout with `regmeta_build/docs/` present, "
...
def _resolve_slug_dir(slug_arg: str | None) -> Path:
    ...
    if resolved is None:
        raise RegmetaError(
            ...
            remediation=(
                "Run from a repo checkout, or `regmeta-build seed-slugs` "
...
def _cmd_seed_slugs(args: argparse.Namespace) -> tuple[dict[str, Any], int]:
    ...
    try:
        ...
        # regmeta-build always emits JSON on stdout, so hints (stderr) are
        # independent of format and only suppressed by --quiet / env.
        suppress_hints = args.quiet or os.environ.get("REGMETA_QUIET") == "1"
...
def _version_line() -> str:
    ...
    return f"regmeta-build v{build_version}"


def _print_usage() -> None:
    w = sys.stderr.write
    w(f"{_version_line()}\n\n")
    w("Build pipeline for the regmeta SQLite databases (maintainer-only).\n\n")
    w("Commands:\n")
    col_w = max(len(syntax) for syntax, _ in _COMMAND_OVERVIEW) + 2
    for syntax, desc in _COMMAND_OVERVIEW:
        w(f"  {syntax:<{col_w}} {desc}\n")
    w("\nRun `regmeta-build <command> --help` for detailed help.\n")
