...
Curated slugs live in per-provider TOML files at ``regmeta_build/fqid_slugs/``.
...
def repo_slug_dir() -> Path | None:
    """Return ``regmeta_build/fqid_slugs/`` from a repo checkout, or ``None``.
    ...
    if updates:
        try:
            ...
            raise _err(
                ...
                "Run `regmeta-build precheck-slugs` to list the colliding "
...
def _assert_no_unslugged(
    ...
    *,
    ...
    raise _err(
        ...
        f"Run `regmeta-build precheck-slugs` to list every missing ID, "
...
def seed_provider_toml(conn: sqlite3.Connection, provider_slug: str) -> str:
    ...
    lines: list[str] = [
        ...
        "# Generated by `regmeta-build seed-slugs`. Hand-review every slug,",
        "# then commit to regmeta_build/fqid_slugs/.",
        "",
    ]
    regs = conn.execute(
...
def seed_classifications_toml(conn: sqlite3.Connection) -> str:
    ...
    lines: list[str] = [
        ...
        "# Generated by `regmeta-build seed-slugs`. Hand-review the slug",
...
def read_snapshot(snapshot_path: Path) -> dict[str, dict[str, str]]:
    ...
    try:
        ...
        raise _err(
            ...
            "Regenerate it with `regmeta-build precheck-slugs --update-snapshot`.",
        ) from exc
