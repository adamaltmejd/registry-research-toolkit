...
# A4.x — see §15 (the A1.3 / A2.1.5 migration steps). Full IR DDL in
...
onto `variable_id` and gains an explicit `register_variant_id` coordinate (A2.1
...
CREATE TABLE variable (
    ...
    is_sensitive         INTEGER NOT NULL DEFAULT 0,  -- A1.2 flags ride the promotion unchanged
...
CREATE TABLE variable_state (
    ...
    register_variant_id              INTEGER NOT NULL REFERENCES register_variant(register_variant_id),  -- delivery coordinate
...
CREATE INDEX idx_variable_state_register_variant ON variable_state(register_variant_id);
...
see §15 (A2.1.5). The `idx_variable_state_uniq` **unique** index above
...
variable's states reference it as their `register_variant_id` coordinate. The
...
| variant (coordinate) | entity / coordinate | A `variant` row (SCB `registervariant`, SOS `deldatamängd`): a **delivery coordinate**, not an identity level. Carried on each `variable_state` (`register_variant_id`) and on `project_data` Sources (`register_variant`). Browsed under its register (§9.5); **not an FQID kind** (§5.2). Carries panel-template + browsing metadata. |
...
variable's `variable_state` rows are filtered to `register_variant_id` matching
...
3. Checks that the FQID resolves to a known `variable` in
   reg_meta, then to a `variable_state` whose `register_variant_id` matches the
...
The PR-sized chunking with checkboxes and the v0.x → Model A rework
map lived in `MIGRATION_PLAN.md`, a scoped self-deleting tracker
removed when the migration shipped; the per-PR landing history is now
in git. This section is the surviving design-level narrative.
...
the git log for the full per-PR breakdown).
...
- **A2.1.5 Two-level table restructure + stored variable slug (respec; supersedes #133).** The structural prerequisite for triage and lineage — pulled earlier per the maintainer red-line, because A2.2/A2.4 cannot run on the A2.1 `(register_id, var_id)` schema (no `variable` table to write sibling variables to; no variable hierarchy to descend). Promote `variable` in place — it keeps its name (already the register-scoped `variable` table on `main`), gaining a synthetic `variable_id AUTOINCREMENT` PK + a register-unique `slug` (A1.2 flags + shared metadata ride along). **Natural key = `(register_id, slug)` UNIQUE**; **`provider_key` is a NON-unique join hint** (plain index — triage siblings share one source key; maintainer red-line on DP1). Re-parent `variable_state` onto `variable_id` with an explicit `register_variant_id` coordinate; `value_set_version_label NOT NULL DEFAULT ''` so the state-uniqueness index bites. Stored-variable-slug population (auto-derive from the latest kolumnnamn, cross-variant tiebreak = highest `regver_id` then lexically smallest; §5.3); the `[variable.<...>]` TOML keys collapse from 3-part `<reg>.<variant>.<var>` to register-scoped 2-part `<reg>.<var>`. Also **demote the `variable_same_as` table to variable grain** (drop `*_variant`/`*_period`, delete the `auto:var_id_match` `(N choose 2)` auto-derive — redundant once variables are register-scoped; keep the name) **and rewrite the `_resolve_binding_via_same_as` query to match, in lockstep** — the live resolver selects `b_variant`/`b_period`, so dropping those columns without the query rewrite would raise `no such column` on any `same_as` fallback during A2.1.5–A2.5. That lets A2.4's lineage linker read a variable-grain table with no dependency on A2.6. Resolver still reads `variable_instance` for direct resolution here. On the 4.x line (promotion + re-parent + column move + `same_as` grain demotion).
- **A2.2 Build-time triage (two-level).** Per §5.7. Requires A2.1.5 (the `variable` table). Kolumnnamn-primary discriminator that **folds** same-concept representations (grain/vintage/coding → one variable with `value_set_version_label`-discriminated states) and **splits** genuinely-different concepts into **distinct `variable` rows** — auto-derives sibling slugs for splits, emits `variable_related_to` edges (variable grain), and persists the discriminator maps (§5.7) so later delivery rows resolve to the right sibling (split) or state (fold). Catches the 11,945 same-year collision buckets across 3,281 distinct variable triples (~2.69% of `(variable, year)` buckets in current SCB data; reproduced from §5.7); ~200-300 cases need manual TOML curation (variable-slug TOML overrides for ambiguous suffixes). In-flight PR #132 reworks onto this shape (siblings become distinct variables). **Also lands the narrow interim binding-resolver flip pulled from A2.5** — binding resolution moves off the `variable_instance` `provider_key` join onto `variable_state` keyed by `variable_id`, so split siblings resolve through their own state (still returning the interim 5-seg `ResolvedVariableBinding`; `cvid`/lineage stay on `variable_instance` until A2.4/A2.7). The siblings and the flip must ship together — minting siblings on the `provider_key`-join resolver silently mis-resolves them.
...
- **A2.4 `variable_state_lineage` interval-overlap join (variable-grain matching).** Per §5.6. Requires A2.1.5 (reads `variable.source_register_id`, descends the variable hierarchy). Replace `link_consumer_side_bindings` with the new linker. Add `variable_state_lineage` + `variable_state_lineage_warning` tables to DDL. Source-side matching traverses variable-grain `same_as` (`variable_set_via_same_as`). **Note:** the `same_as` table is already variable-grain (demoted in A2.1.5), so this linker reads it directly — **no dependency on A2.6** (A2.6 only rewrites the resolver-side traversal, which this build-time linker never calls). That is what keeps the A2.4↔A2.6 gate acyclic. Source-variant pinning is **TOML-only** — `[lineage_defaults]` and `[lineage."<consumer>.<variable>"]` blocks, no `variable_source_lineage` SQL table. Old `via_source_id` column populated in parallel for transition.
- **A2.5 Catalog API shift.** `Catalog.resolve(fqid)` **flips in place** to the §5.10 longitudinal semantics (returns `ResolvedVariable` — variable metadata + states each tagged with their variant + variable-grain edges). The v0.x per-cvid behavior is **deleted**, not deprecated — pre-v1 policy allows the break. Add `resolve_at(fqid, period, *, variant=None, value_set_version=None)`, `states`, `predecessors`, `successors`, `related`, `lineage`, `lineage_warnings` per §5.10. Reads the A2.1.5 tables; the *binding-FQID* parse stays on the **v0.11 5-seg** grammar until A2.6 (the Model-A 4-seg form was specced in #126 but never implemented, so the interim is the shipped 5-seg, not 4-seg). The **narrow binding-resolver flip** onto `variable_state` (keyed by `variable_id`) already landed in **A2.2** (it had to ship with the triage siblings); A2.5 builds on that flipped read path, replacing the interim `ResolvedVariableBinding` with the `ResolvedVariable` aggregate. Webapp endpoints in §9.5 still on v0.x grammar (binding leaves still 5-seg); A2.6 flips both straight to 3-seg.
- **A2.6 Drop period & variant from FQID grammar (resolver flip).** The grammar flip — the table restructure (incl. the `same_as` grain demotion **and the lockstep `_resolve_binding_via_same_as` column-shape rewrite**) already landed in A2.1.5. (1) **Drop BOTH period and variant from the FQID** (4→3 seg `provider/register/slug`; variant becomes a navigational register sub-resource, §5.2). Update parser, emitter, slug-loading. (2) **Resolver flip:** `_resolve_binding_direct` parses 3-seg and reads the stored `slug` by exact match (no derive-at-resolve), joining `variable_state` through `variable_id` + `register_variant_id` + period. (3) Drop ~1,264 `register_version` slug entries; drop the `register_version` table (prose → reg-meta-docs, artifacts → provenance). (4) **`same_as` start node from the 3-seg parse** — the `variable_same_as` table and its `_resolve_binding_via_same_as` reader are already variable-grain from A2.1.5; A2.6 only feeds the query's start-node triple from the new 3-seg parse instead of the interim 5-seg form (no further column or query change). (5) Webapp catalog endpoints flip to 3-seg binding leaves + variant sub-resource + new sub-endpoints. v0.x reg_meta clients break; pre-v1 policy says no shim. UNFROZEN sentinel is active so the slug TOML rewrite is a regular commit.
...
- **A4.3 SOS adapter.** `reg_meta_build/sources/sos.py` `SOSAdapter` consumes the 13 SOS workbooks via the existing parser, emits IR. **Merges same-named variables across deldatamängder into one `variable`** (one `IRVariableState` per `(deldatamängd, period)` with the deldatamängd as the `register_variant_id` coordinate; §5.1); splits into distinct variables only on a genuine meaning conflict via triage (§5.7). Handles variant synthesis for LSS/BU/SOL (`_default`). Kodlista state-era parsing per §5.7. MFR IVF_klinik entity-registry heuristic. ~2,300 IR rows.
