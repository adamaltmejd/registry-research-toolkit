...
lanes` reads the agent's text from stdin, frames it, and splices it as its own region.
...
def render_lanes_block(content: str) -> str:
    ...
    in the marked region + the do-not-edit header, mirroring `render_block`'s framing. No
    timestamp — the issue-edit history records when; the body stays diff-stable.
    """
    return "\n".join(
        [
            LANES_START,
            "## Lanes — generated",
            "",
            "<!-- Generated by `/plan-lanes` (run by `/issue-pulse` on a material tick); "
...
def main() -> int:
    ...
    ap.add_argument("--write-lanes", action="store_true",
                    help="splice the ranked-lanes block (read from stdin) into the epic body")  # fmt: skip
    args = ap.parse_args()

    # --write-lanes only frames + splices agent-supplied text; it needs no issue corpus,
    # so handle it before the (expensive) fetches below.
    if args.write_lanes:
        content = sys.stdin.read()
        if not content.strip():
            print("--write-lanes: empty stdin (nothing to splice)", file=sys.stderr)
            return 2
        block = render_lanes_block(content)
        current = (
