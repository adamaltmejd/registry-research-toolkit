...
def test_parse_minimal(stats_path: Path):
    ...
    assert result.files[0].file_name == "persons.csv"
    assert len(result.files[0].columns) == 6
    assert result.files[0].row_count == 1000
...
def test_column_types(stats_path: Path):
    result = parse_stats(stats_path)
    cols = {c.column_name: c for c in result.files[0].columns}
...
def test_empty_files(tmp_path: Path):
    p = tmp_path / "bad.json"
    p.write_text(json.dumps({"contract_version": "1.0.0", "files": []}))
    with pytest.raises(StatsValidationError, match="No files"):
        parse_stats(p)
...
def test_no_columns_in_file(tmp_path: Path):
    p = tmp_path / "bad.json"
    data = {
        "contract_version": "1.0.0",
        "files": [{"file_name": "x.csv", "row_count": 10, "columns": []}],
    }
    p.write_text(json.dumps(data))
