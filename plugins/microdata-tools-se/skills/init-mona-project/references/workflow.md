...
- that setup happens in two phases:
  - Phase 1 creates the local scaffold and the MONA extract bundle
...
> (`mock_data_wizard_extract.py`); after you run that on MONA and bring back
> `stats.json`, I generate mock data and finish the project docs and repo setup.
>
> To start, I need:
>
> 1. project slug
...
### Build the MONA extract bundle

```bash
cd {projdir} && mock-data-wizard build-bundle
```

This writes `mock_data_wizard_extract.py` in `{projdir}/`. The bundle
...
> **Next steps on MONA:**
>
> 1. Upload `mock_data_wizard_extract.py` to MONA via **My Files** and move it
...
> 5. Review `stats.json` in Notepad++ before export. It must contain only
...
> 7. Place `stats.json` in `{projdir}/` and return to this skill.
...
> bundle (`mock_data_wizard_extract.py`); the next step is the MONA round-trip
> for `stats.json`.
...
After successful generation, delete `{projdir}/mock_data_wizard_extract.py`
